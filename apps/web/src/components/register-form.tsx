      ...
      if (axios.isAxiosError(error)) {
        if (error.response) {
          ...
          if (errorMessage === "Email already registered") {
            ...
            setError(errorMessage || "Terjadi kesalahan saat registrasi. Silakan coba lagi.")
      ...
      >
        {({ isSubmitting }) => (
          <Form className="flex flex-col items-center justify-center h-full px-4 sm:px-12 bg-white w-full max-w-md mx-auto">
            ...
            <div className="h-5 mb-4 w-full sm:w-80">{error && <p className="text-red-500 text-sm">{error}</p>}</div>
