  ...
  return (
    <div className="flex items-center justify-center min-h-screen bg-[#f6f5f7] font-['Poppins',sans-serif]">
      <div className="relative overflow-hidden w-[768px] max-w-full min-h-[500px] bg-white rounded-[25px] shadow-[0_14px_28px_rgba(0,0,0,0.25),0_10px_10px_rgba(0,0,0,0.22)]">
        <div className={`max-sm:hidden absolute top-0 h-full transition-all duration-600 ease-in-out ${isLoginActive ? 'left-0 w-1/2 z-2' : 'left-1/2 w-1/2 z-5'}`}>
          {isLoginActive ? <LoginForm /> : <RegisterForm />}
        </div>
        <div className='md:hidden'><LoginForm /></div>
        <div className={`max-sm:hidden absolute top-0 left-1/2 w-1/2 h-full overflow-hidden transition-transform duration-600 ease-in-out z-100 ${isLoginActive ? '' : '-translate-x-full'}`}>
