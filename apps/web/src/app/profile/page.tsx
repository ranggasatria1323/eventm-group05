...
function ProfilePage() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      formData.append("birthdate", profile.birthdate ? new Date(profile.birthdate).toISOString() : "")
      formData.append("gender", profile.gender || "")
      if (profile.image instanceof File) {
        formData.append("image", profile.image)
      }

      const response = await axios.put(`${process.env.NEXT_PUBLIC_BASE_API_URL}profile`, formData, {
        headers: {
          Authorization: `Bearer ${token}`,
      ...
      <div className="max-w-4xl mx-auto py-8 px-4">
        ...
        <div className="bg-white rounded-lg shadow p-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            {[
              { label: "Nama", name: "name", value: profile.name, readOnly: true },
              { label: "Email", name: "email", value: profile.email, readOnly: true },
              { label: "Tipe Pengguna", name: "userType", value: profile.userType, readOnly: true },
              { label: "Nomor Telepon", name: "phoneNumber", value: profile.phoneNumber, readOnly: false, type: "tel" },
              { label: "Tanggal Lahir", name: "birthdate", value: profile.birthdate, readOnly: false, type: "date" },
              { label: "Jenis Kelamin", name: "gender", value: profile.gender, readOnly: false, type: "select" },
            ].map((field) => (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-8">
            <div className="bg-blue-50 border border-blue-200 rounded-lg p-6 hover:bg-blue-100 hover:shadow-md transition duration-200">
              ...
              <div className="mt-4 flex items-center justify-between">
                <span className="text-xl font-bold text-blue-900">{profile.referralCode}</span>
                <button
                  onClick={() => {
            ...
            <div className="bg-green-50 border border-green-200 rounded-lg p-6 hover:bg-green-100 hover:shadow-md transition duration-200">
              ...
              <div className="mt-4 flex items-center">
                <span className="text-3xl font-bold text-green-900">{profile.points}</span>
